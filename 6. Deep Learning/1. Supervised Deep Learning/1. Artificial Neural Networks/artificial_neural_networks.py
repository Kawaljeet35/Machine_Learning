...
# units = no of hidden neurons we want and relu = rectifier activation fn
ann.add(tf.keras.layers.Dense(units = 6, activation = 'relu'))
# Adding the second hidden layer
ann.add(tf.keras.layers.Dense(units = 6, activation = 'relu'))
# Adding the output layer
# Activation fn sigmoid to get output as a probability (softmax in case of non binary o/p)
# units = 1 because we want a binary output
...
# Transforming the features because model trained on transformed model
print(ann.predict(sc.transform([[1, 0, 0, 600, 1, 40, 3, 60000, 2, 1, 1, 50000]])))
# Predicting the test set results
# Creating the prediction of X_test
y_pred = ann.predict(X_test)
# To get o/p value as 1 if predicted value  > 0.5
y_pred = (y_pred > 0.5)
